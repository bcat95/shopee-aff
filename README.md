# 📚 Tài liệu API Shopee Affiliate (Shopee Affiliate API Documentation)

Nguồn gốc: [https://affiliate.shopee.vn/open_api/list](https://affiliate.shopee.vn/open_api/list)

Tài liệu này cung cấp thông tin chi tiết về các API của chương trình Shopee Affiliate, cho phép bạn lấy thông tin sản phẩm, tạo link affiliate và theo dõi đơn hàng một cách tự động.
...
4.  **`signature`:** Một chuỗi mã hóa được tạo ra từ `secret_key` và các tham số của yêu cầu để đảm bảo tính bảo mật. Shopee sẽ cung cấp cách tạo signature.
